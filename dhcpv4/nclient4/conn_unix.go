...
// Fragments are dropped here too, since only a whole datagram can be parsed.
func dhcpClientFilter(clientPort uint16) []bpf.Instruction {
	const (
		// More-fragments plus the fragment offset; don't-fragment is not fragmentation.
		fragmentBits = 0x2000 | 0x1fff
		// acceptAll is the snap length for a frame the filter passes.
		acceptAll = 262144
	)
	return []bpf.Instruction{
		bpf.LoadAbsolute{Off: versIHL, Size: 1},                                         // version + IHL
		bpf.ALUOpConstant{Op: bpf.ALUOpAnd, Val: 0xf},                                   // A = IHL
		bpf.JumpIf{Cond: bpf.JumpLessThan, Val: ipv4MinimumSize / 4, SkipTrue: 7},       // short header: drop
		bpf.LoadAbsolute{Off: protocol, Size: 1},                                        // IP protocol
		bpf.JumpIf{Cond: bpf.JumpNotEqual, Val: uint32(udpProtocolNumber), SkipTrue: 5}, // not UDP: drop
		bpf.LoadAbsolute{Off: flagsFO, Size: 2},                                         // flags + fragment offset
		bpf.JumpIf{Cond: bpf.JumpBitsSet, Val: fragmentBits, SkipTrue: 3},               // a fragment: drop
		bpf.LoadMemShift{Off: versIHL},                                                  // X = IHL * 4
		bpf.LoadIndirect{Off: udpDstPort, Size: 2},                                      // UDP destination port
		bpf.JumpIf{Cond: bpf.JumpEqual, Val: uint32(clientPort), SkipTrue: 1},           // matches: accept
		bpf.RetConstant{Val: 0},                                                         // drop
		bpf.RetConstant{Val: acceptAll},                                                 // accept
	}
}
